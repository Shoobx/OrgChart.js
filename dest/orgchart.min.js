...
var OrgChart = function () {
  ...
  _createClass(OrgChart, [{
    ...
    key: 'addZoomControls',
    value: function addZoomControls(appendTo) {
      var _this = this;

      var zoomContainerEl = document.createElement('div');

      zoomContainerEl.classList.add('zoom-container');

      var zoomInEl = document.createElement('div');
      ...
      zoomInEl.addEventListener('click', function () {
        return _this._setChartScale(_this.chart, true);
      });
      zoomContainerEl.appendChild(zoomInEl);

      var zoomOutEl = document.createElement('div');
      ...
      zoomOutEl.addEventListener('click', function () {
        return _this._setChartScale(_this.chart, false);
      });
    ...
    value: function _buildJsonDS(li) {
      ...
      if (li.querySelector('ul')) {
        Array.from(li.querySelector('ul').children).forEach(function (el) {
          ...
          subObj.children.push(_this2._buildJsonDS(el));
        });
      }
      return subObj;
    }
    ...
    value: function showSiblings(node, direction) {
      ...
      this._one(siblings[0].querySelector('.slide'), 'transitionend', function () {
        ...
        siblings.forEach(function (sib) {
          _this5._removeClass(Array.from(sib.querySelectorAll('.slide')), 'slide');
        });
        if (this._isInAction(node)) {
    ...
    value: function hideSiblings(node, direction) {
      ...
      siblings.forEach(function (sib) {
        if (sib.querySelector('.spinner')) {
          _this6.chart.dataset.inAjax = false;
        }
      });
      ...
      this._one(animatedNodes[0], 'transitionend', function (event) {
        ...
        sibs.forEach(function (sib) {
          _this7._addClass(Array.from(sib.querySelectorAll('.lines')), 'hidden');
          _this7._addClass(Array.from(sib.querySelectorAll('.nodes')), 'hidden');
          _this7._addClass(Array.from(sib.querySelectorAll('.verticalNodes')), 'hidden');
        });
    ...
    key: '_addShowAllBottom',
    value: function _addShowAllBottom(node) {
      var showAllEl = document.createElement('div');
      ...
      var iconGroupEl = document.createElement('i');

      iconGroupEl.classList.add('fa', 'fa-group');
      showAllEl.append(iconGroupEl);

      var iconDoubleEl = document.createElement('i');
    ...
    value: function showChildren(node) {
      ...
      this._one(descendants[0], 'transitionend', function (event) {
        _this9._removeClass(descendants, 'slide');
        if (_this9._isInAction(node)) {
          _this9._switchVerticalArrow(node.querySelector('.bottomEdge'));
        }
      }, this);
    ...
    value: function _clickBottomEdge(event) {
      ...
      if (childrenState.exist) {
        ...
        if (childrenState.visible) {
          ...
          node.querySelectorAll('.show-all.bottom').forEach(function (el) {
            return el.remove();
          });
        }
      } else {
      ...
      if (nodeChart.parentNode.nodeName === 'TD') {
        ...
        that.buildHierarchy.call(that, that.chart, nodeData, 0, function () {
          if (++nodeCount === siblingCount) {
            ...
            var temp3 = _this11._siblings(nodeChart.parentNode);
    ...
    value: function _clickHorizontalEdge(event) {
      ...
      if (siblingsState.exist) {
        ...
        if (siblings.some(function (el) {
          ...
          return _this12._isVisible(node) && node.classList.contains('slide');
        })) {
          return;
        }
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (!nodeData.root && opts.showAvatars) {
          ...
          if (nodeData.avatarUrl && nodeData.avatarUrl.length > 0) {
            ...
            var name = nodeData[opts.firstLine];
            var firstLetter = name[0];
            var secondLetter = '';
            var secondLetterIndex = name.indexOf(' ');
        ...
        if (!nodeData.root && opts.firstLine) {
          ...
          if (opts.secondLine && nodeData[opts.secondLine] && nodeData[opts.secondLine].length > 0) {
            var contentEl2 = document.createElement('div');
          ...
          if (nodeData.border_color) {
            ...
            var borderColor = nodeData.border_color ? 'border-' + opts.borderColors[nodeData.border_color] : '';

            var borderLegendEl = document.createElement('div');
        ...
        if (opts.verticalDepth && level + 2 > opts.verticalDepth || Number(flags.substr(2, 1))) {
          if (level + 1 >= opts.verticalDepth && Number(flags.substr(2, 1))) {
            ...
            if (Number(flags.substr(2, 1)) && !nodeData.root) {
              var _toggleBtn = document.createElement('i'),
                  _icon = level + 1 >= opts.depth ? 'plus' : 'minus';

              _toggleBtn.setAttribute('class', 'toggleBtn edge bottomEdge bottomEdge-click fa fa-' + _icon + '-square');
              nodeDiv.appendChild(_toggleBtn);
            }
          }
        }
    ...
    value: function _loopChart(chart) {
      ...
      if (chart.children[3]) {
        Array.from(chart.children[3].children).forEach(function (el) {
          ...
          subObj.children.push(_this13._loopChart(el.firstChild));
        });
      }
      return subObj;
    }
    ...
    key: '_setChartScale',
    value: function _setChartScale(chart, zoomIn) {
      var scales = [0.25, 0.33, 0.5, 0.67, 0.75, 0.8, 0.9, 1, 1.1, 1.25, 1.5, 1.75, 2, 2.5, 3];
      var scale = scales[7];
      ...
      if (chart.style.transform.length === 0) {
        ...
        var matrix = JSON.parse(chart.style.transform.replace(/^\w+\(/, "[").replace(/\)$/, "]"));
        var scaleIndex = scales.indexOf(matrix[0]);
